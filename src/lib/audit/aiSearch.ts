...
export async function auditAISearch(url: string, businessName: string, city: string, industry: string): Promise<CategoryScore> {
  const checks = await checkAISearchSignals(url, businessName, city, industry);
  ...
  if (checks.hasStructuredData) {
    ...
    actions.push({
      ...
      tool2Upsell: "AI Search Optimizer",
    });
  }
...
async function checkAISearchSignals(url: string, businessName: string, city: string, industry: string) {
  try {
