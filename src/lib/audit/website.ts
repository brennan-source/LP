import { CategoryScore, ActionItem } from "@/types/audit";
...
export async function auditWebsite(url: string): Promise<CategoryScore> {
  ...
  if (psi.score >= 0.9) {
    ...
    actions.push({
      ...
      tool2Upsell: "Site Speed Optimizer",
    });
  } else {
  ...
  if (psi.mobileScore >= 0.9) {
    ...
    actions.push({
      ...
      tool2Upsell: "Mobile Conversion Optimizer",
    });
  } else {
  ...
  if (siteChecks.hasLiveChat) {
    ...
    actions.push({
      ...
      tool2Upsell: "24/7 AI Chat Agent",
    });
  }
  score += featureScore;
