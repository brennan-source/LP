    ...
    <div className={cn("bg-white rounded-2xl border border-stone-200 overflow-hidden", className)}>
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-sm text-stone-500 truncate">{category.summary}</p>
        </div>

        <div className="flex items-center gap-3 shrink-0">
          <div className="text-right hidden sm:block">
            <div className="text-xs text-stone-400 mb-0.5">vs competitors</div>
          ...
          {category.estimatedRevenueLoss && category.estimatedRevenueLoss > 0 ? (
            <div className="text-right hidden md:block">
              <div className="text-xs text-stone-400 mb-0.5">est. loss/mo</div>
        ...
        <div className="border-t border-stone-100 p-5 space-y-5">
          <div className="space-y-2">
            <div className="flex items-center justify-between text-xs text-stone-500">
              ...
              <span className="font-medium text-stone-700">{category.score}/100</span>
            </div>
            ...
            <div className="flex items-center justify-between text-xs text-stone-500">
              ...
              <span className="font-medium text-stone-700">{category.competitorAvg}/100</span>
            </div>
            <div className="h-2 bg-stone-100 rounded-full overflow-hidden">
              <div className="h-full rounded-full bg-stone-300" style={{ width: `${category.competitorAvg}%` }} />
            </div>
          </div>

          <div>
            <h4 className="text-xs font-semibold text-stone-500 uppercase tracking-wider mb-2">Findings</h4>
            <ul className="space-y-1">
              {category.details.map((detail, i) => (
                <li key={i} className="text-sm text-stone-600">{detail}</li>
              ))}
            </ul>
          </div>
          ...
          {category.actions.length > 0 && (
            <div>
              <h4 className="text-xs font-semibold text-stone-500 uppercase tracking-wider mb-3">Recommended Actions</h4>
                  ...
                  return (
                    <div key={i} className={cn("border rounded-lg p-3", styles.bg)}>
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <span className="text-sm font-semibold text-stone-800">{action.title}</span>
                      </div>
                      <p className="text-xs text-stone-600 mb-1">{action.description}</p>
                      <p className="text-xs font-medium text-stone-700">Impact: {action.estimatedImpact}</p>
                    </div>
                  );
                })}
              </div>
