import { GrowthScoreResults, BusinessInput, ScoreCategory, ActionItem, Competitor } from "@/types/growth-score";
import { scoreToGrade, normalizeUrl } from "@/lib/utils";
...
export async function runGrowthScoreAudit(submissionId: string, input: BusinessInput): Promise<GrowthScoreResults> {
  ...
  const roadmap = buildRoadmap(categories);
