...
function getPrisma(): PrismaClient {
  if (!_prisma) {
    const g = globalThis as unknown as { _agencyPrisma?: PrismaClient };
    _prisma = g._agencyPrisma ?? createPrismaClient();
    if (process.env.NODE_ENV !== "production") g._agencyPrisma = _prisma;
  }
  return _prisma;
}
