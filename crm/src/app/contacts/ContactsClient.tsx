      ...
      <div className="bg-slate-800 rounded-xl border border-slate-700 overflow-hidden">
        {loading ? (
          ...
          <div className="p-8 text-center text-slate-400">
            {search || stageFilter ? "No contacts match your filters." : "No contacts yet. Add one or run the scraper."}
          </div>
        ) : (
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-700">
                ...
                <th className="text-left px-4 py-3 text-slate-400 font-medium hidden lg:table-cell">Industry</th>
                <th className="text-left px-4 py-3 text-slate-400 font-medium">Stage</th>
                <th className="text-left px-4 py-3 text-slate-400 font-medium">Products</th>
              </tr>
            </thead>
            <tbody>
              {contacts.map((contact, i) => (
                <tr
                  ...
                  className={`hover:bg-slate-700/50 transition ${
                    i < contacts.length - 1 ? "border-b border-slate-700/50" : ""
                  }`}
                >
                  ...
                  <td className="px-4 py-3 text-slate-300 hidden lg:table-cell">
                    {contact.industry ?? <span className="text-slate-500">—</span>}
                  </td>
                  <td className="px-4 py-3">
                    <span
                      className={`inline-flex text-xs px-2 py-0.5 rounded-full font-medium ${
                        STAGE_COLORS[contact.stage] ?? "bg-slate-600 text-slate-200"
                      }`}
                    >
