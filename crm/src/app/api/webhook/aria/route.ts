...
export async function POST(request: Request) {
  try {
    ...
    const { email, businessName, city, state, industry, assessmentId } = body;
    ...
    // Upsert ContactProduct for aria
    ...
    await prisma.activity.create({
      data: {
        contactId: contact.id,
        type: "purchase",
        description: "Purchased Aria AI assessment",
        metadata: assessmentId ? JSON.stringify({ assessmentId }) : null,
      },
    });
